        ...
        <div className="mt-7 grid gap-5 lg:grid-cols-[300px_1fr]">
          ...
          <div className="space-y-5">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              {sections.map(({ key, title, icon: Icon, color, bg }) => <section key={key} className="rounded-3xl border border-ink/10 bg-white p-6"><span className={`grid size-10 place-items-center rounded-xl ${bg} ${color}`}><Icon size={20} /></span><h2 className="mt-5 font-serif text-2xl font-semibold">{title}</h2>{result[key].length ? <ul className="mt-4 space-y-3">{result[key].map((item, i) => <li key={i} className="flex gap-3 text-sm leading-6 text-ink/65"><span className="mt-2 size-1.5 shrink-0 rounded-full bg-green" />{item}</li>)}</ul> : <p className="mt-4 text-sm text-ink/45">Không có nhận xét trong mục này.</p>}</section>)}
            </div>
            <section className="rounded-3xl bg-green p-6 text-white sm:p-8"><span className="flex items-center gap-3"><BriefcaseBusiness /><h2 className="font-serif text-2xl font-semibold">Vị trí phù hợp</h2></span><div className="mt-5 flex flex-wrap gap-2">{result.suitableRoles.map((role) => <span key={role} className="rounded-full bg-white/15 px-4 py-2 text-sm font-semibold">{role}</span>)}</div></section>
          </div>
        </div>
      </div>
    </main>
