...
export default function Home() {
  return (
    <main className="min-h-screen overflow-hidden bg-[radial-gradient(circle_at_15%_10%,#dff4e8_0,transparent_28%),radial-gradient(circle_at_90%_35%,#f7d8cf_0,transparent_24%)]">
      <Header />
      <section className="mx-auto grid max-w-6xl items-center gap-12 px-5 pb-24 pt-12 sm:px-8 lg:grid-cols-[1fr_0.9fr] lg:pt-20">
        <div>
          <span className="inline-flex items-center gap-2 rounded-full bg-mint px-3 py-1.5 text-xs font-bold uppercase tracking-[0.16em] text-green">AI CV Reviewer <ArrowDown size={13} /></span>
          <h1 className="mt-6 max-w-2xl font-serif text-5xl font-semibold leading-[0.98] tracking-tight sm:text-7xl">CV tốt hơn bắt đầu từ một góc nhìn <span className="italic text-green">rõ hơn.</span></h1>
      ...
      <section className="border-t border-ink/10 bg-white/70">
        <div className="mx-auto grid max-w-6xl gap-4 px-5 py-16 sm:px-8 md:grid-cols-3">
          {features.map(({ icon: Icon, title, text }, index) => <article key={title} className="rounded-3xl border border-ink/10 bg-white p-6"><span className="mb-8 flex items-center justify-between"><Icon className="text-green" /><small className="font-serif text-xl text-ink/25">0{index + 1}</small></span><h2 className="font-serif text-2xl font-semibold">{title}</h2><p className="mt-2 text-sm leading-6 text-ink/55">{text}</p></article>)}
        </div>
      </section>
    </main>
  );
